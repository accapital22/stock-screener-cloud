...
def sort_results_for_pdf(results):
    """Sort results for PDF report with timeframe hierarchy: Daily -> Weekly -> Monthly"""
    if not results:
        return []
    
    # Create timeframe priority mapping
    timeframe_priority = {
        'Daily (1Y)': 1,
        'Weekly (2Y)': 2, 
        'Monthly (5Y)': 3
        ...
        if stock.get('options_details') and len(stock['options_details']) > 0:
            ...
            for option in stock['options_details']:
                flattened_results.append({
                    ...
                    'timeframe_priority': timeframe_priority.get(stock['timeframe_label'], 99),
    ...
    # Apply the sorting hierarchy: Timeframe -> Delta -> Option_Price -> Symbol
    df_sorted = df.sort_values(['timeframe_priority', 'has_options', 'delta', 'option_price', 'symbol'], 
    ...
    try:
        ...
        if results:
            ...
            if sorted_results:
                ...
                results_data = [[
                    'Stock', 'Timeframe', 'Stock_Price', 'Bank_Price', 'Option_Strike', 'Option_Price', 
                ...
                for i, result in enumerate(sorted_results):
                    ...
                    if current_timeframe is not None and result['timeframe_label'] != current_timeframe:
                        results_data.append([''] * 13)  # Blank row for timeframe separation
                        # Add timeframe header row
                        timeframe_header = [''] * 13
                        timeframe_header[0] = f"=== {result['timeframe_label'].upper()} ==="
                        results_data.append(timeframe_header)
                        results_data.append([''] * 13)  # Another blank row
                    
                    current_timeframe = result['timeframe_label']
                    
                    # Add blank row when delta changes (only for stocks with options)
                    if (current_delta is not None and result['delta'] != current_delta and 
                        result['has_options'] and result['timeframe_label'] == current_timeframe):
                        results_data.append([''] * 13)  # Blank row for delta separation
                ...
                col_widths = [0.5*inch, 0.8*inch, 0.6*inch, 0.6*inch, 0.6*inch, 0.6*inch, 
                             0.4*inch, 0.7*inch, 0.8*inch, 0.8*inch, 0.5*inch, 0.3*inch, 0.5*inch]
...
def create_futures_pdf_report(results, params):
    ...
    try:
        ...
        if results:
            ...
            results_table = Table(results_data, colWidths=[1.0*inch, 1.0*inch, 0.8*inch, 0.8*inch, 0.7*inch, 0.8*inch, 0.7*inch])
...
def send_email_with_attachment(pdf_buffer, recipient_emails, report_type="stocks"):
    ...
    with tab1:
        # Initialize stock screener INSIDE the tab
        ...
        # Main stock screening button with cancel - FIXED VERSION
        ...
        with col1:
            if st.button("🚀 Run Stock Screener", type="primary", use_container_width=True, key="stock_screener_btn"):
                ...
                st.session_state.cancel_stock_screening = False
                # Use a placeholder to maintain state during screening
                screening_placeholder = st.empty()
                
                with screening_placeholder.container():
        ...
        with col2:
            # Show cancel button if screening is active
    ...
    with tab2:
        # Initialize futures screener INSIDE the tab
        ...
        # Main futures screening button with cancel - FIXED VERSION
        col1, col2 = st.columns([3, 1])
